...
async function computeProjectCount(
  monthDate: Date,
...
export async function computeStudentsTotal(
  ...
  const aggregate = await withTimeout(
    client.monthlyClassroomAttendance.aggregate({
      where: {
  ...
  if (!Number.isFinite(total))
    throw new Error('Invalid reported student enrollment total');
  return total;
}
...
function classroomWindowFilter(monthDate: Date) {
  ...
  return {
    AND: [
      {
        OR: [
          { dateCreated: { lt: endExclusive } },
          { dateCreated: null, createdAt: { lt: endExclusive } },
        ],
      },
...
export async function computeClassroomsTotal(monthDate: Date): Promise<number> {
  return withTimeout(
    prisma.classroom.count({ where: classroomWindowFilter(monthDate) }),
    KPI_COMPUTATION_TIMEOUT,
  );
}
...
export async function computeSeniorShare(monthDate: Date): Promise<number> {
  ...
  const [total, senior] = await withTimeout(
    Promise.all([
...
export function personsTrainedMonthFilter(
  monthDate: Date,
...
export async function computeStudentsPassedX(
  ...
  return withTimeout(
    client.legacyBoardResult.count({
...
export async function getMonthlyFinanceAggregate(
  ...
  if (!Number.isFinite(amount))
    throw new Error('Invalid monthly finance amount');
...
export async function upsertAuto(
  ...
  if (!kpiId || !Number.isFinite(value))
    throw new Error('Invalid KPI auto value');
  const normalizedMonth = normalizeMonthDate(month);
  await withTimeout(
    client.kPIMonthlyValue.upsert({
      where: {
        kpiId_month_source: { kpiId, month: normalizedMonth, source: 'AUTO' },
      },
      update: { value },
...
export async function computeCentresTotal(monthDate: Date): Promise<number> {
  ...
  return withTimeout(
    prisma.centre.count({
      where: {
        dateAssociated: { lt: endExclusive },
        OR: [{ dateLeft: null }, { dateLeft: { gte: start } }],
      },
    }),
    KPI_COMPUTATION_TIMEOUT,
  );
...
export async function computeFinances(monthDate: Date): Promise<{
  ...
  const currentCutoff = currentFy
    ? nextMonthStart(targetMonth) < fiscalEndExclusive(currentFy.endDate)
      ? nextMonthStart(targetMonth)
      : fiscalEndExclusive(currentFy.endDate)
    : null;
  const pastCutoff = pastFy ? fiscalEndExclusive(pastFy.endDate) : null;
  const [currentRevenue, currentExpenditure, pastRevenue, pastExpenditure] =
    await Promise.all([
      currentFy && currentCutoff
        ? getFinanceAggregate(
            'CREDIT',
            currentFy.id,
            currentFy.startDate,
      ...
      currentFy && currentCutoff
        ...
        : 0,
      pastFy && pastCutoff
        ? getFinanceAggregate('CREDIT', pastFy.id, pastFy.startDate, pastCutoff)
        : 0,
      pastFy && pastCutoff
        ? getFinanceAggregate('DEBIT', pastFy.id, pastFy.startDate, pastCutoff)
        : 0,
    ]);
