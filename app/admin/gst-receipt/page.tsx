...
type GstRow = {
  ...
  grandTotal: number;
  createdAt: string;
  // Included for preview/edit logic if needed
  items?: any[];
  billToGstin?: string;
  dateOfSupply?: string;
...
export default function GstReceiptsPage() {
  ...
  const handleCreateOrUpdate = async (data: any) => {
    // If we have an ID, it's an update (you might need to implement PUT in API if desired)
    // For now, let's assume this handles CREATE as per your request
    // If you need UPDATE, you'd check `editMode` and call PUT /api/admin/gst-receipt/[id]

    // NOTE: Your previous request only showed POST for create.
    // If you need update logic, you'd switch method here based on `editMode`.
    const method = editMode === "create" ? "POST" : "POST"; // Placeholder if PUT not ready
    const endpoint = "/api/admin/gst-receipt"; // Placeholder
  ...
  // Helper to open edit (if you implement edit later) or just view details
  // For now, we reuse form modal logic but you might need to fetch full details first
  // since LIST API excludes items.
  const openEdit = async (row: GstRow) => {
    // Ideally fetch full row with items here before opening
    // const res = await fetch(`/api/admin/gst-receipt/${row.id}`);
    // const fullData = await res.json();

    // For this example, assuming row has enough or we just use what we have
    setEditMode("edit");
    setSelectedRow(row);
    setFormOpen(true);
  };
  ...
  const openPreview = async (row: GstRow) => {
    // Fetch full details including items for preview
    try {
      const res = await fetch(`/api/admin/gst-receipt/${row.id}`);
