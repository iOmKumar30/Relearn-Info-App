...
export async function createVoucherFromTxn(txn: any, serialNo: string) {
  try {
    ...
    if (isExist) {
      console.error("Voucher number already exists");
      return { success: false, error: "Voucher number already exists" };
    }
...
export async function createGstReceiptFromTxn(txn: any, serialNo: string) {
  try {
    ...
    const isExist = await prisma.gstReceipt.findUnique({
    ...
    const receipt = await prisma.gstReceipt.create({
      data: {
        ...
        items: [
          {
            description: txn.reason || txn.description || "Service/Goods",
            amount: parseFloat(txn.amount),
...
export async function createDonationFromTxn(txn: any, serialNo: string) {
  try {
    ...
    const isExist = await prisma.donation.findUnique({
      where: { receiptNumber: receiptNo },
    });
    if (isExist) {
      console.error("Receipt number already exists");
      return { success: false, error: "Receipt number already exists" };
    }
