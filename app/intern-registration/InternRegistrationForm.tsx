...
const INTEREST_OPTIONS = [
  ...
  'Awareness through performing arts',
...
export default function InternRegistrationForm() {
  ...
  const handleDropdownChange = (
    key: keyof CustomFieldsState,
    value: string,
  ) => {
    if (value === 'Other') {
      setCustomFields((prev) => ({ ...prev, [key]: true }));
      update(key, '');
    } else {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (typeof data?.paymentToken !== 'string' || !data.paymentToken) {
        throw new Error(
          'Unable to start your internship activation. Please contact Relearn Foundation.',
        );
      }
      ...
      sessionStorage.setItem(
        'intern-registration-payment-token',
        data.paymentToken,
      );
  ...
  return (
    <main className="min-h-screen bg-slate-50 px-4 py-8 sm:px-6 lg:py-12">
      ...
      <section className="mx-auto max-w-5xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl shadow-slate-200/60">
          ...
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
            ...
            <div className="border-slate-200 sm:border-l sm:pl-7">
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
                Share your details to apply for an internship. Our team will
                review your registration and contact you.
              </p>
            </div>
          </div>
        </header>
        ...
        <form
          onSubmit={submit}
          className="space-y-8 px-5 py-7 sm:px-10 sm:py-10"
        >
          {success && (
            <div
              role="status"
            ...
            >
              Your internship registration has been submitted successfully. We
          ...
          <fieldset className="space-y-4 border-t border-slate-200 pt-8">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                <FieldLabel htmlFor="institution">Current institute</FieldLabel>
                <input
                  id="institution"
                  maxLength={200}
              ...
              <div>
                <FieldLabel htmlFor="previousInstitute">
                  Previous institute
                </FieldLabel>
                <input
                  ...
                  onChange={(e) => update('previousInstitute', e.target.value)}
                  className={inputClassName}
                />
              </div>

              <div>
                <FieldLabel htmlFor="ongoingCourse">Ongoing course</FieldLabel>
                <select
                  id="ongoingCourse"
                  value={
                    customFields.ongoingCourse ? 'Other' : form.ongoingCourse
                  }
                  onChange={(e) =>
                    handleDropdownChange('ongoingCourse', e.target.value)
                  }
                  className={inputClassName}
                >
                  <option value="">Select an option</option>
                ...
                {customFields.ongoingCourse && (
                  <input
                    id="ongoingCourse_other"
                    maxLength={200}
                    placeholder="Please specify your ongoing course"
                    value={form.ongoingCourse}
                    onChange={(e) => update('ongoingCourse', e.target.value)}
              ...
              <div>
                <FieldLabel htmlFor="educationCompleted">
                  Education completed
                </FieldLabel>
                <select
                  id="educationCompleted"
                  value={
                    customFields.educationCompleted
                      ? 'Other'
                      : form.educationCompleted
                  }
                  onChange={(e) =>
                    handleDropdownChange('educationCompleted', e.target.value)
                  }
                  className={inputClassName}
                >
                  <option value="">Select an option</option>
                ...
                {customFields.educationCompleted && (
                  <input
                    id="educationCompleted_other"
                    maxLength={200}
                    placeholder="Please specify your completed education"
                    value={form.educationCompleted}
                    onChange={(e) =>
                      update('educationCompleted', e.target.value)
                    }
                    className={`${inputClassName} mt-3`}
                  />
                )}
              ...
              <div className="md:col-span-2">
                <FieldLabel htmlFor="areasOfInterest">
                  Areas of interest
                </FieldLabel>
                <select
                  id="areasOfInterest"
                  value={
                    customFields.areasOfInterest
                      ? 'Other'
                      : form.areasOfInterest
                  }
                  onChange={(e) =>
                    handleDropdownChange('areasOfInterest', e.target.value)
                  }
                  className={inputClassName}
                >
                  <option value="">Select an option</option>
          ...
          <fieldset className="space-y-4 border-t border-slate-200 pt-8">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
              ...
              <div>
                <FieldLabel htmlFor="workingMode">
                  Preferred working mode
                </FieldLabel>
                <select
                  id="workingMode"
                  value={form.workingMode}
                  onChange={(e) => update('workingMode', e.target.value)}
                  className={inputClassName}
                >
                  <option value="">Select an option</option>
              ...
              <div>
                <FieldLabel htmlFor="preferredHoursPerDay">
                ...
                <input
                  ...
                  value={form.preferredHoursPerDay}
                  onChange={(e) =>
                    update('preferredHoursPerDay', e.target.value)
                  }
                  className={inputClassName}
                />
              </div>
              <label className="flex min-h-11 items-center gap-3 rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-700 md:col-span-2 lg:col-span-3">
                <input
                  type="checkbox"
                  checked={form.associatedAfter}
                  onChange={(e) => update('associatedAfter', e.target.checked)}
                  className="h-4 w-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                />
                I am interested in future association with Relearn Foundation
          ...
          <div className="border-t border-slate-200 pt-8">
            <p className="mb-4 text-sm leading-6 text-slate-600">
              Please complete the verification to protect this form from
              automated submissions. Your registration does not create an
            ...
            <button
              ...
              className="inline-flex w-full items-center justify-center rounded-lg bg-blue-700 px-5 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-200 disabled:cursor-not-allowed disabled:bg-slate-400 sm:w-auto"
            >
              {isSubmitting
