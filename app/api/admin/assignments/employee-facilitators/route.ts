...
async function isAdmin(userId?: string) {
  ...
  const u = await prisma.user.findUnique({
    where: { id: userId },
    select: { roleHistory: { where: { endDate: null }, select: { role: { select: { name: true } } } } },
  });
...
export async function GET(req: Request) {
  ...
  if (!employeeUserId && !facilitatorId) {
    return new NextResponse("Bad Request: employeeUserId or facilitatorId required", { status: 400 });
  }
...
export async function POST(req: Request) {
  ...
  if (!employeeUserId || !facilitatorId) {
    return new NextResponse("employeeUserId and facilitatorId are required", { status: 400 });
  }
  ...
  const created = await prisma.facilitatorEmployee.create({
    ...
    select: { id: true, employeeUserId: true, facilitatorId: true, startDate: true },
  });
