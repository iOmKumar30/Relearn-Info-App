...
export default function HonoraryMembersPage() {
  ...
  const fetchMembers = useCallback(async () => {
    try {
      ...
      toast.error("Could not load honorary members data");
    } finally {
      setLoading(false);
    }
  }, [
  ...
  const handleCreate = async (formData: any) => {
    ...
    try {
      const res = await fetch("/api/admin/members/honorary", {
        method: "POST",
      ...
      toast.success("Honorary member created successfully", {
        id: loadingToast,
      });
      await fetchMembers();
    } catch (err: any) {
  ...
  const handleUpdate = async (id: string, formData: any) => {
    ...
    try {
      const res = await fetch(`/api/admin/members/honorary/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(formData),
      });
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/members/honorary/${deleteRow.id}`, {
        method: "DELETE",
      });
      if (!res.ok) throw new Error(await res.text());

      toast.success("Honorary member deleted successfully", {
        id: loadingToast,
      });
      setDeleteOpen(false);
      setDeleteRow(null);
