...
// Ensure this matches your actual cleaned filename for Life Members
const CSV_PATH = path.join(process.cwd(), "Life-Members-RELF.cleaned.csv");
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (!member) {
        member = await tx.member.create({
          data: {
            userId: user.id,
            memberType: MemberType.LIFE, // <--- Target Type changed to LIFE
        ...
        // If member exists, ensure type is updated to LIFE
        if (member.memberType !== MemberType.LIFE) {
          await tx.member.update({
            where: { id: member.id },
            data: { memberType: MemberType.LIFE },
          });
        }
      }

      // Note: No fees are seeded here as requested
    });
  }
